...
const directorName = computed(() => {
  if (Array.isArray(movie.value.credits?.crew)) {
    const directorObj = movie.value.credits.crew.find(
  ...
  if (movie.value.director) {
    if (typeof movie.value.director === "object") {
      return movie.value.director.name || null;
    }
...
const topCast = computed(() => {
  ...
  if (Array.isArray(movie.value.credits?.cast)) {
    rawCast = movie.value.credits.cast;
  } else if (Array.isArray(movie.value.cast)) {
    rawCast = movie.value.cast;
  }
